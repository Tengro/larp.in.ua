...
api_v1_urlpatterns = [
    path(
        ...
        include(("larp_in_ua.apps.accounts.api.v1.urls", "accounts"), namespace="api-v1-accounts"),
    )
]

django_telegrambot_path = [re_path(r'^', include('django_telegrambot.urls')), ]

urlpatterns = admin_urlpatterns + api_v1_urlpatterns + django_telegrambot_path

# enable Swagger
if "SWAGGER" in settings.LARP_IN_UA_FEATURES:
    ...
    api_v1_schema_view = get_schema_view(
        openapi.Info(
            title="larp_in_ua",
            default_version="v1",
            description="larp_in_ua API v1 description",
        ),
        public=True,
